# train_chroma_ot.mojo — Chroma1-HD LoRA training, OneTrainer-FAITHFUL recipe.
#
# This is train_chroma_real.mojo RE-TARGETED to the OneTrainer "#chroma LoRA
# 24GB" preset (OneTrainer 423c3b36; effective config = default_values() +
# preset overlay, materialized as /home/alex/OneTrainer/configs/
# chroma_eri2wt_100_baseline.json; oracle 100-step baseline 2026-07-16 in
# /home/alex/mojodiffusion/output/chroma_ot_eri2wt_100_baseline). The block/
# stack math, offload loader, save surface and loop plumbing are REUSED
# unchanged; train_chroma_real.mojo is left intact (anima precedent:
# serenitymojo/training/train_anima_ot.mojo).
#
# THE OT DELTAS (vs the chroma.rs-anchored train_chroma_real.mojo):
#  1. lora_alpha 16 -> 1.0 (scale = alpha/rank = 1/16). Preset leaves
#     TrainConfig default lora_alpha=1.0 (TrainConfig.py:1144); the oracle
#     artifact's alpha keys are 1.0 (measured, 912-key dump).
#  2. lr 1e-4 -> 3e-4 with LINEAR WARMUP 200 then constant: preset
#     learning_rate=3e-4; TrainConfig default learning_rate_warmup_steps=200,
#     scheduler CONSTANT. Measured oracle ramp (tfevents): optimizer step t
#     (1-based) uses lr = 3e-4 * t/200, capped at 1 — lr[s0]=1.5e-6,
#     lr[s50]=7.65e-5, lr[s99]=1.5e-4. constant_lr() ramps (step+1)/warmup, so
#     this driver passes (optimizer_step - 1).
#  3. timestep: logit-normal(shift=1.15) -> OneTrainer INVERTED_PARABOLA
#     discrete multinomial (noising_weight=7.7, noising_bias=0, shift=1.0;
#     ModelSetupNoiseMixin._get_timestep_discrete). Sampler =
#     schedule.mojo inverted_parabola_weight_table +
#     sample_timestep_idx_from_weight_table (gated distribution-level:
#     scripts/check_inverted_parabola_dist.py PASS 2026-07-16, table 3.3e-7
#     vs torch-f32, 2M-draw histogram cos 0.999787). sigma=(idx+1)/1000 and
#     t_model=idx/1000 conventions are UNCHANGED (verified against
#     ModelSetupFlowMatchingMixin._add_noise_discrete + timestep/1000).
#  4. trained module set: full 418-slot carrier -> OneTrainer layer_filter
#     "attn,ff.net" = 304 modules (ff_context/proj_mlp/proj_out/distilled
#     guidance excluded; measured from the oracle artifact). Implementation:
#     the full carrier still runs the forward (excluded adapters keep B=0
#     forever: their grads are ZEROED before accum/clip/AdamW, so m=v=0 and
#     wd*0=0 -> function-identical to not training them), the grad global-norm
#     is therefore the 304-set norm, and every save/state/resume goes through
#     the *_for_layer_filter surface (912-key artifacts, bit-exact round-trip
#     gate chroma_lora_ot_real_artifact_roundtrip.mojo PASS 2026-07-16).
#  5. FIXED_SIGMA_SMOKE False: real per-step samples/noise/timesteps.
#  6. batch: OT preset batch_size=2 -> the existing TRUE batch-2 row-stacked
#     device arm (batch_size=2 in config), stride-1 cache pairing (recorded
#     divergence: OT epoch-shuffles disjoint pairs).
#
#  7. T5 pad-KEY mask (2026-07-16): OT passes cat(text_mask, ones) ->
#     outer-product mask; implemented as the krea2-style row permutation
#     [txt_valid | img | txt_pad] + cuDNN real_len suffix mask in
#     _chroma_sdpa_fwd/bwd (chroma_block_device.mojo), real_lt threaded through
#     the device stack; pad outputs/grads exact 0. Gate:
#     parity/chroma_padmask_parity.mojo (masked flash vs truncated math,
#     fwd/bwd cos>=0.999, pad rows exact 0). Host stack has no mask arm ->
#     CHROMA_HOST_STACK fails loud here.
#
# RECORDED (not fixed here) divergence axes vs OneTrainer:
#   - noise/timestep RNG streams are distribution-level, not bit-level.
#   - base weights fp8_e4m3-resident (policy MJ-1065) vs OT bf16.
#
# Recipe scalars (configs/chroma_ot_24gb.json): lr=3e-4, warmup=200, rank=16,
#   alpha=1.0, INVERTED_PARABOLA w=7.7 b=0 shift=1.0, clip_grad_norm=1.0,
#   VAE shift=0.1159 scale=0.3611, AdamW b1=0.9 b2=0.999 eps=1e-8 wd=0.01.
#
# Build (serenity-trainer): pixi run chroma-ot-trainer-build
# Run: target/serenity_chroma_ot_trainer <config.json> [steps]
...
from serenitymojo.models.chroma.chroma_stack_lora import (
    ...
    save_chroma_lora_for_layer_filter, save_chroma_lora_state_for_layer_filter,
    chroma_layer_filter_slot_mask,
)
...
from serenitymojo.training.schedule import (
    inverted_parabola_weight_table, sample_timestep_idx_from_weight_table,
)
...
# ── recipe (configs/chroma_ot_24gb.json = OneTrainer "#chroma LoRA 24GB") ────
comptime RANK = 16
comptime ALPHA = Float32(1.0)
comptime LR = Float32(3.0e-4)
comptime LR_WARMUP_STEPS = 200
comptime TIMESTEP_SHIFT = Float32(1.0)
comptime TIMESTEP_WEIGHT = Float32(7.7)   # OT noising_weight
comptime TIMESTEP_BIAS = Float32(0.0)     # OT noising_bias
comptime TSD_INVERTED_PARABOLA_TAG = 5    # io reader int for INVERTED_PARABOLA
comptime OT_LAYER_FILTER = "attn,ff.net"  # OT preset layer_filter (304 modules)
...
comptime CACHE_DIR = "/home/alex/datasets/eri2wt_chroma_edv2_512"
comptime LORA_DIR = "/home/alex/mojodiffusion/output/chroma_ot_recipe"
comptime DEFAULT_CONFIG = "/home/alex/mojodiffusion/serenitymojo/configs/chroma_ot_24gb.json"
...
def validate_chroma_train_config(cfg: TrainConfig) raises:
    if cfg.adapter_algo == TRAIN_ADAPTER_ALGO_LOCON:
        ...
        cfg.adapter_algo == TRAIN_ADAPTER_ALGO_LOKR
        or cfg.adapter_algo == TRAIN_ADAPTER_ALGO_LOHA
        or cfg.adapter_algo == TRAIN_ADAPTER_ALGO_DORA
        or cfg.adapter_algo == TRAIN_ADAPTER_ALGO_OFT
    ):
        raise Error(
            String("Chroma OT trainer: the OneTrainer 24GB preset is plain LoRA; ")
            + adapter_algo_name(cfg.adapter_algo)
            + String(" is out of scope here — use train_chroma_real")
        )
    ...
    if not _close_f32(cfg.lr, LR, Float32(1.0e-9)):
        raise Error("Chroma OT trainer learning_rate does not match compiled constant 3e-4")
    if cfg.lr_warmup_steps != LR_WARMUP_STEPS:
        raise Error(
            String("Chroma OT trainer requires lr_warmup_steps=") + String(LR_WARMUP_STEPS)
            + String(" (OT default warmup); parsed ") + String(cfg.lr_warmup_steps)
        )
    if not _close_f32(cfg.timestep_shift, TIMESTEP_SHIFT):
        raise Error("Chroma OT trainer timestep_shift must be 1.0 (OT preset)")
    if cfg.timestep_distribution != TSD_INVERTED_PARABOLA_TAG:
        raise Error(
            String("Chroma OT trainer requires timestep_distribution=INVERTED_PARABOLA; parsed tag ")
            + String(cfg.timestep_distribution)
        )
    if not _close_f32(cfg.timestep_noising_weight, TIMESTEP_WEIGHT):
        raise Error("Chroma OT trainer noising_weight must be 7.7 (OT preset)")
    if not _close_f32(cfg.timestep_noising_bias, TIMESTEP_BIAS):
        raise Error("Chroma OT trainer noising_bias must be 0.0 (OT preset)")
...
def _save_chroma_lora_ema(
    ...
    _ = save_chroma_lora_for_layer_filter(ema_set, String(OT_LAYER_FILTER), ema_path, ctx)
...
def main() raises:
    ...
    print("=== Chroma (chroma1-hd) OT-RECIPE LoRA training loop (OneTrainer #chroma LoRA 24GB) ===")
    ...
    # ── OT delta #4: layer_filter "attn,ff.net" per-slot trainability mask ──
    var trained_mask = chroma_layer_filter_slot_mask(
        NUM_DOUBLE, NUM_SINGLE, String(OT_LAYER_FILTER)
    )
    var n_trained = 0
    for i in range(len(trained_mask)):
        if trained_mask[i]:
            n_trained += 1
    if len(trained_mask) != n_adapters or n_trained != 304:
        raise Error(
            String("Chroma OT trainer: layer_filter mask expected 304 trained of ")
            + String(n_adapters) + String(" slots; got ") + String(n_trained)
            + String(" of ") + String(len(trained_mask))
        )
    print("[lora] OT layer_filter=", String(OT_LAYER_FILTER),
          " -> trained", n_trained, "of", n_adapters,
          "slots (excluded grads zeroed; excluded B stays exactly 0)")
    ...
    # OT delta #3: INVERTED_PARABOLA bucket weights, built once (N=1000).
    var tsd_table = inverted_parabola_weight_table(
        TIMESTEP_WEIGHT, TIMESTEP_BIAS, TIMESTEP_SHIFT, NUM_TRAIN_TIMESTEPS
    )
    ...
    for k in range(1, run_steps + 1):
        ...
        # OT delta #7 (pad-mask fix 2026-07-16): valid T5 rows; >= N_TXT disables
        # the mask (matches OT attention_mask=None when the text has no padding).
        var real_lt = t5_seq if t5_seq < N_TXT else N_TXT
        var t5_cache = _load_chroma_cache_tensor(st, String("t5_embed"), ctx)
        var t5_flat = _host_f32_for_step_math(t5_cache, ctx)
        ...
        if FIXED_SIGMA_SMOKE:
            ...
            # OT delta #3: discrete multinomial draw (index directly).
            sigma_idx = sample_timestep_idx_from_weight_table(
                SEED_BASE + step_seed, tsd_table
            )
        ...
        if dora_active:
            ...
            var step_lr = ot_lr_for_optimizer_step(train_cfg, k - 1)
        ...
        if use_b2:
            ...
            var real_lt1 = t5_seq1 if t5_seq1 < N_TXT else N_TXT
            ...
            if FIXED_SIGMA_SMOKE:
                ...
                # OT delta #3: discrete multinomial draw (index directly).
                sigma_idx1 = sample_timestep_idx_from_weight_table(
                    SEED_BASE + step_seed1, tsd_table
                )
            ...
            raise Error(
                "Chroma OT trainer: the OT recipe requires the DEVICE stack with the "
                "T5 pad-key mask; unset CHROMA_HOST_STACK (host stack has no mask arm)."
            )
        ...
        # OT delta #4: zero excluded-slot grads BEFORE accum/clip/AdamW so the
        # global norm, clip and update see exactly the 304-module OT set.
        for i in range(n_adapters):
            if not trained_mask[i]:
                for j in range(len(grads.d_a[i])):
                    grads.d_a[i][j] = Float32(0.0)
                for j in range(len(grads.d_b[i])):
                    grads.d_b[i][j] = Float32(0.0)
        ...
        # OT delta #2: constant_lr ramps (step+1)/warmup; pass optimizer_step-1
        # so optimizer step t uses lr = base*t/warmup (measured oracle tfevents).
        var step_lr = ot_lr_for_optimizer_step(train_cfg, optimizer_step - 1)
        if lokr_active:
        ...
        if chroma_should_save_checkpoint(train_cfg, k):
            ...
            if lokr_active:
                ...
                _ = save_chroma_lora_for_layer_filter(lora, String(OT_LAYER_FILTER), save_path, ctx)
                ...
                _ = save_chroma_lora_state_for_layer_filter(lora, String(OT_LAYER_FILTER), state_path, ctx)
        ...
        if sample_enabled and should_sample_completed_step(sample_cadence, k):
            if chroma_should_save_before_sample(sample_cadence, k, saved_this_step):
                ...
                if lokr_active:
                    ...
                    _ = save_chroma_lora_for_layer_filter(lora, String(OT_LAYER_FILTER), sample_path, ctx)
                    ...
                    _ = save_chroma_lora_state_for_layer_filter(lora, String(OT_LAYER_FILTER), sample_state, ctx)
    ...
    if trains and (last_loss == last_loss):
        ...
        if lokr_active:
            ...
            _ = save_chroma_lora_for_layer_filter(lora, String(OT_LAYER_FILTER), lora_out, ctx)
            if train_cfg.ema_enabled:  # T1.B EMA sibling
                _save_chroma_lora_ema(ema, lora, lora_out, n_adapters, ctx)
            var state_out = lora_out + String(".state.safetensors")
            _ = save_chroma_lora_state_for_layer_filter(lora, String(OT_LAYER_FILTER), state_out, ctx)
