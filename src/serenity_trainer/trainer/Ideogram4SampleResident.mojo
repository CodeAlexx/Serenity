...
# (`Ideogram4LoraSet`) — by running the verified resident LoRA forward inside a
...
# WITH the LoRA currently being trained. ideogram4_lora_train_forward_resident
# (Ideogram4LoRATrainStep.mojo:446) applies the LoRA on top of the resident base
# (it routes the block stack through ideogram4_stack_lora_forward_resident), so
# calling it IS the model+LoRA forward. Its returned `.velocity` is the toolkit
# velocity convention: -( image rows ) in [1,128,GH,GW] F32 (see
# Ideogram4Predict.ideogram4_predict_velocity / the train forward, lines 493-496).
#
...
# The trainer cache holds pre-encoded DATASET-caption llm_features [1,NT,53248],
# NOT arbitrary sample-prompt features. v1 reuses a cached caption's llm_features
# as the COND conditioning (option (b) in the build request): it exercises the
# real denoise + CFG + decode + PNG path with conditioning already resident in
# VRAM — no extra Qwen3-VL load, no tokenizer wiring (both of which would compete
# with the two resident DiT trunks + LoRA + Adam state for memory and add risk).
# The UNCOND is the zeroed text features the OT/inference sampler uses for CFG
# (neg_llm all-zero -> empty conditioning). The SampleCadence prompt STRINGS are
# carried for forward-compat; for v1 the prompt index selects which cached caption
# conditions the sample (documented at the call site). Swapping in a real
# encode(prompt) later is a drop-in replacement of `cond_llm` only.
...
from serenity_trainer.trainer.Ideogram4LoRATrainStep import (
    ideogram4_lora_train_forward_resident,
)
...
#
# NOTE on the comptime split: ideogram4_lora_train_forward_resident is
...
def ideogram4_sample_resident[NT: Int, GH: Int, GW: Int](
    ...
    for step in range(n_steps - 1, -1, -1):
        ...
        var cond_fwd = ideogram4_lora_train_forward_resident[NT, GH, GW](
            weights, z, t_flow, cond_llm, loras, ctx
        )
        # UNCOND pass: same trunk + LoRA, zeroed text conditioning.
        var uncond_fwd = ideogram4_lora_train_forward_resident[NT, GH, GW](
            weights, z, t_flow, uncond_llm, loras, ctx
        )

        # asymmetric CFG on the (negated) velocities:
        #   v = cfg*pos_vel + (1-cfg)*neg_vel
        var v = add(
            mul_scalar(cond_fwd.velocity, cfg, ctx),
            mul_scalar(uncond_fwd.velocity, Float32(1.0) - cfg, ctx),
            ctx,
        )
